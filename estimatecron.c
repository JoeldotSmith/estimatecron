...
//Structure to store values of each command
struct commands {
    char name[100];
    char execTime[10]; // note char not int to easily handle "*"
    char minute[60];
    char hour[24];
    char day[31];
    char month[12];
...
// dayofweek algorithm https://en.wikipedia.org/wiki/Determination_of_the_day_of_the_week
// Tomohiko Sakamoto algorithm
// Under Methods in Computer code
...
//Gives time in minutes of the time a command should be terminated in a day
...
int main(int argc, const char *argv[]) {
    ...
    if (cronF == NULL){
        ...
        while (fgets(input, sizeof(input), estiF) != NULL){
            ...
            // ignore commented lines in files
        ...
        while (fgets(input, sizeof(input), cronF) != NULL){
            ...
            if (strncmp(&c, hashtag, 1)!=0){
                ...
                //splits text into tokens and sets variables
                char *token = strtok(input, " ");
                ...
                //goes through each command in commandList array and checks if the name matches then copies data into command struct for that command
    ...
    //sets number of days in the month and sets a solid reference number for month
    ...
    for (int day = 1; day <= dMonth; day++){
        ...
        for (int hour = 0; hour <= 24; hour++){
            ...
            for (int minutes = 0; minutes <= 60; minutes++){
                ...
                //checks for max num of commands running
                            ...
                            if (atoi(commandList[i].dayWeek) == dayofweek(day, actualMonth+1, 2022) || strcmp(commandList[i].dayWeek, star)==0){
                                if (atoi(commandList[i].hour) == hour || strcmp(commandList[i].hour, star)==0){
                                    if (atoi(commandList[i].minute) == minutes || strcmp(commandList[i].minute, star)==0){
                                        ...
                                        // increases num of times called for command
